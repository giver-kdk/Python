...
class TeamsAutoTyper:
    ...
    def save_times(self):
        ...
        with open(r"D:\CODES\Python\Python-Projects\AutoClocker\times.json", "w") as f:
            json.dump(times, f)
    ...
    def load_times(self):
        ...
        try:
            ...
            with open(r"D:\CODES\Python\Python-Projects\AutoClocker\times.json", "r") as f:
                times = json.load(f)

            # Schedule GUI updates using after
            for day in self.days_disabled.keys():
                # Type-In time
                type_in_time = times.get(f"{day}_in", "09:30")
                self.root.after(0, self.update_time_entry, self.time_entries[f"{day}_in"], type_in_time)

                # Type-Out time
                type_out_time = times.get(f"{day}_out", "18:30")
                self.root.after(0, self.update_time_entry, self.time_entries[f"{day}_out"], type_out_time)

        except FileNotFoundError:
            # If the file doesn't exist, use default times
            for day in self.days_disabled.keys():
                # Type-In time (default)
                self.root.after(0, self.update_time_entry, self.time_entries[f"{day}_in"], "09:30")

                # Type-Out time (default)
                self.root.after(0, self.update_time_entry, self.time_entries[f"{day}_out"], "18:30")

    def update_time_entry(self, entry, time_value):
        """Helper function to update the time entry widget."""
        entry.delete(0, tk.END)
        entry.insert(0, time_value)
